    ...
    if input_type == "fasta":
        for alignment in sam_pysam.fetch():
            if alignment.reference_name is None:
                record = SeqRecord(Seq(alignment.query_sequence), id=alignment.query_name, name=alignment.query_name)
...
def expectation_maximization_iterations(log_p_rgs, db_ids, lli_thresh, input_threshold):
    ...
    while True:
        ...
        if not 0.9 <= freq_sum <= 1.1:
            raise ValueError("f sums to {}, rather than 1".format(freq_sum))

        # confirm log likelihood increase
        log_likelihood_diff = round(updated_log_likelihood, 2) - round(total_log_likelihood, 2)
        ...
        if log_likelihood_diff < lli_thresh:
            ...
            if freq_thresh < input_threshold:
                ...
                freq_set_thresh, updated_log_likelihood, p_sgr = expectation_maximization(log_p_rgs, freq)
...
def freq_to_lineage_df(freq, tsv_output_path, taxonomy_df, assigned_count, unassigned_count, counts=False):
    ...
    counts(boolean): True if include estimated_counts in output .tsv file
    returns(df): pandas df with lineage and abundances for values in f
    """
    # add tax lineage for values in freq
    results_df = pd.DataFrame(zip(list(freq.keys()) + ["unassigned"], list(freq.values()) + [0]), columns=["tax_id", "abundance"]).set_index("tax_id")
    ...
    if counts:
        counts_series = pd.concat([(results_df["abundance"] * assigned_count)[:-1], pd.Series(unassigned_count)], ignore_index=True)
...
def generate_alignments(in_file_list, out_basename, database):
    ...
    if filetype == ".sam":
        ...
        return input_file
    sam_align_file = "{}_align_alignments.sam".format(out_basename)
    db_sequence_file = os.path.join(database, "species_taxid.fasta")

    subprocess.check_output(
        "minimap2 -ax {} -t {} -N {} -p .9 -K {} {} {} -o {}".format(
            args.type, args.threads, args.N, args.K, db_sequence_file, input_file, sam_align_file
        ),
        shell=True,
    )
...
def create_reduced_fasta(fasta_dict, db_name):
    ...
    for seq, tid_dict in fasta_dict.items():
        for taxid, descriptions in tid_dict.items():
            records += [SeqRecord(seq, id="{}:{}:{}".format(taxid, db_name, count), description="{}".format(descriptions))]
            count += 1
    return records
...
def collapse_rank(path, rank):
    ...
    if "estimated_counts" in df_emu.columns:
        df_emu_copy = df_emu[["abundance", "estimated_counts"] + keep_ranks]
    ...
    for file in os.listdir(dir_path):
        ...
        if file_extension == ".tsv" and "rel-abundance" in file:
            ...
            if rank in df_sample.columns and metric in df_sample.columns:
                ...
                df_sample_reduced = (
                    df_sample_reduced.groupby(keep_ranks_sample, dropna=False).sum().reset_index()
...
if __name__ == "__main__":
    ...
    abundance_parser.add_argument("input_file", type=str, nargs="+", help="filepath to input nt sequence file")
    abundance_parser.add_argument(
        "--type", "-x", choices=["map-ont", "map-pb", "sr"], default="map-ont", help="short-read: sr, Pac-Bio:map-pb, ONT:map-ont [map-ont]"
    )
    abundance_parser.add_argument("--min-abundance", "-a", type=float, default=0.0001, help="min species abundance in results [0.0001]")
    abundance_parser.add_argument(
        "--db",
        type=str,
        default=os.environ.get("EMU_DATABASE_DIR"),
        help="path to emu database containing: names_df.tsv, " "nodes_df.tsv, species_taxid.fasta, unqiue_taxids.tsv [$EMU_DATABASE_DIR]",
    )
    abundance_parser.add_argument("--N", "-N", type=int, default=50, help="minimap max number of secondary alignments per read [50]")
    abundance_parser.add_argument("--K", "-K", type=int, default=500000000, help="minibatch size for minimap2 mapping [500M]")
    abundance_parser.add_argument("--output-dir", type=str, default="./results", help="output directory name [./results]")
    abundance_parser.add_argument("--output-basename", type=str, help="basename for all emu output files [{input_file}]")
    abundance_parser.add_argument("--keep-files", action="store_true", help="keep working files in output-dir")
    abundance_parser.add_argument("--keep-counts", action="store_true", help="include estimated read counts in output")
    abundance_parser.add_argument("--keep-read-assignments", action="store_true", help="output file of read assignment distribution")
    abundance_parser.add_argument("--output-unclassified", action="store_true", help="output unclassified sequences")
    abundance_parser.add_argument("--threads", "-t", type=int, default=3, help="threads utilized by minimap [3]")

    build_db_parser = subparsers.add_parser("build-database", help="Build custom Emu database")
    build_db_parser.add_argument("db_name", type=str, help="custom database name")
    build_db_parser.add_argument("--sequences", type=str, required=True, help="path to fasta of database sequences")
    build_db_parser.add_argument("--seq2tax", type=str, required=True, help="path to tsv mapping species tax id to fasta sequence headers")
    taxonomy_group = build_db_parser.add_mutually_exclusive_group(required=True)
    taxonomy_group.add_argument("--ncbi-taxonomy", type=str, help="path to directory containing both a names.dmp and nodes.dmp file")
    taxonomy_group.add_argument("--taxonomy-list", type=str, help="path to .tsv file mapping full lineage to corresponding taxid")

    collapse_parser = subparsers.add_parser("collapse-taxonomy", help="Collapse emu output at specified taxonomic rank")
    collapse_parser.add_argument("input_path", type=str, help="emu output filepath")
    collapse_parser.add_argument("rank", type=str, help="collapsed taxonomic rank")

    combine_parser = subparsers.add_parser("combine-outputs", help="Combine Emu rel abundance outputs to a single table")
    combine_parser.add_argument("dir_path", type=str, help="path to directory containing Emu output files")
    combine_parser.add_argument("rank", type=str, help="taxonomic rank to include in combined table")
    combine_parser.add_argument("--split-tables", action="store_true", help="two output tables:abundances and taxonomy lineages")
    combine_parser.add_argument("--counts", action="store_true", help="counts rather than abundances in output table")
    ...
    if args.subparser_name == "abundance":
        ...
        if not args.db:
            raise ValueError("Database not specified. " "Either 'export EMU_DATABASE_DIR=<path_to_database>' or " "utilize '--db' parameter.")
        df_taxonomy = pd.read_csv(os.path.join(args.db, "taxonomy.tsv"), sep="\t", index_col="tax_id", dtype=str)
        ...
        log_prob_rgs, counts_unassigned, counts_assigned = log_prob_rgs_dict(SAM_FILE, log_prob_cigar_op, longest_align_dict, locs_p_cigar_zero)
        f_full, f_set_thresh, read_dist = expectation_maximization_iterations(log_prob_rgs, db_species_tids, 0.01, args.min_abundance)
        freq_to_lineage_df(f_full, "{}_rel-abundance".format(out_file), df_taxonomy, counts_assigned, counts_unassigned, args.keep_counts)
